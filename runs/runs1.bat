:: to execute this batch file, cd to project directory and type "runs\runs.bat"

@REM python main.py val_per1_best
@REM python main_results.py val_per1_best all 1300 1400

@REM python main.py val_per2_17
@REM python main_results.py val_per2_17 all 1300 1400

python main.py val_per1_best
python main.py val_per3_best

python main_results.py val_per1_best all 1300 1400
python main_results.py val_per3_best all 1300 1400
